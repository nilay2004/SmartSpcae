...
module BP3D.Three {
  export class Edge {
    ...
    private init() {
      this.edge.redrawCallbacks.add(() => this.redraw());
      this.controls.cameraMovedCallbacks.add(() => this.updateVisibility());
      this.updateTexture();
      this.updatePlanes();
      this.addToScene();
    }
    ...
    private makeWall(start: {x: number, y: number}, end: {x: number, y: number}, transform: THREE.Matrix4, invTransform: THREE.Matrix4, material: THREE.Material) {
